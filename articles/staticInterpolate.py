...
def main():
    ...
    for html_file in glob.glob(os.path.join(args.dir, '*html')):
        soup = BeautifulSoup(
            open('/Users/mac/Desktop/Programming/kona-django-nextjs-test/cartopia/articles/templates/articles/out/index.html'), 'html.parser')
        ...
        django_script = BeautifulSoup(
            "<script>window.django = {is_authenticated: \"{{ request.user.is_authenticated }}\" === \"True\" ? true : false, user_plan: \"{{user_plan}}\"};</script>", 'html.parser')
        head = soup.find('head')
        head.append(django_script)
        search_and_parse(soup, 'link')
        search_and_parse(soup, 'script')
        fout = open(
            '/Users/mac/Desktop/Programming/kona-django-nextjs-test/cartopia/articles/templates/articles/out/index.html', 'wb')
